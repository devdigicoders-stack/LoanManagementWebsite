    ...
    <section className="relative w-full bg-black text-white pt-16 pb-32 lg:pb-40 overflow-x-clip min-h-[500px]">
      ...
      <div className="absolute top-0 right-0 w-full md:w-1/2 h-full z-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-[20%] -right-[10%] w-[150%] h-[150%] bg-black transform rotate-[25deg] shadow-[-20px_0_40px_rgba(0,0,0,0.8)] hidden md:block">
          ...
          <div className="absolute top-0 left-0 w-[12px] h-full bg-gradient-to-b from-[#eef200] to-[#1cf200]"></div>
          
          {/* Decorative stripes mimicking the screenshots */}
          <div className="absolute top-0 -left-[40px] w-[8px] h-full bg-white opacity-20"></div>
          <div className="absolute top-0 -left-[70px] w-[4px] h-full bg-gradient-to-b from-[#eef200] to-[#1cf200] opacity-50"></div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto relative w-full h-full px-6 md:px-10 z-10 flex flex-col justify-center min-h-[400px]">
        ...
        {/* Navigation Arrows */}
        <button onClick={prevSlide} className="absolute left-2 md:-left-4 top-1/2 -translate-y-1/2 z-30 bg-black/40 hover:bg-[#1cf200] border border-white/10 hover:border-transparent p-3 rounded-full text-white hover:text-black transition-all backdrop-blur-md hidden md:flex items-center justify-center">
          <ChevronLeft size={24} />
        </button>
        <button onClick={nextSlide} className="absolute right-2 md:-right-4 top-1/2 -translate-y-1/2 z-30 bg-black/40 hover:bg-[#1cf200] border border-white/10 hover:border-transparent p-3 rounded-full text-white hover:text-black transition-all backdrop-blur-md hidden md:flex items-center justify-center">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="md:w-[55%] flex flex-col items-start text-left z-30 md:pr-4 py-10 relative">
              ...
              <h3 className="italic text-gray-300 text-lg md:text-xl lg:text-2xl font-light mb-3 tracking-wide">
                {slides[current].tag}
              </h3>

              <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold leading-[1.1] mb-8 relative z-30 drop-shadow-md">
                {slides[current].title} <br />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#eef200] to-[#1cf200]">{slides[current].highlight}</span>
              </h1>

              <button className="flex items-center justify-center bg-gradient-to-r from-[#eef200] to-[#1cf200] hover:from-[#d5d900] hover:to-[#17cc00] text-black font-bold py-3 px-8 rounded-lg shadow-[0_4px_20px_rgba(28,242,0,0.4)] transition-transform duration-300 hover:scale-105 z-30 relative">
                Know More
              </button>
            </div>
          ...
          {slides.map((_, index) => (
            <button
              ...
              className={`h-[6px] transition-all duration-300 rounded-[1px] ${
                current === index ? 'bg-gradient-to-r from-[#eef200] to-[#1cf200] w-6 shadow-[0_0_8px_rgba(28,242,0,0.8)]' : 'bg-gray-500 hover:bg-gray-300 w-[6px]'
              }`}
